...
def add_lip(face_path):
	model_loaded = False
	shutil.rmtree("ai_core/wav2lip/output_preHD", ignore_errors=
				True)
	import os
	...
	for file_audio in file_names:
		...
		gen = datagen(mel_chunks, face_path)
		...
		for i, (img_batch, mel_batch, frames, coords) in enumerate(tqdm(gen, 
			...
			if model_loaded is False:
				...
				model_loaded = True
			if i == 0:
				frame_h, frame_w = next(read_frames(face_path=face_path)).shape[:-1]
		...
		command = 'ffmpeg -y -i {} -i {} -strict -2 -q:v 1 {}'.format(file_audio, 'ai_core/wav2lip/source/temp/result.avi', "ai_core/wav2lip/output_preHD/" + str(time.time()) + ".mp4")
		...
		if args.save_frames and args.save_as_video:
			...
			command = 'ffmpeg -y -i {} -i {} -strict -2 -q:v 1 {}'.format(file_audio, 'ai_core/wav2lip/source/temp/gt.avi', args.gt_path)
			subprocess.call(command, shell=platform.system() == 'Windows')

			command = 'ffmpeg -y -i {} -i {} -strict -2 -q:v 1 {}'.format(file_audio, 'ai_core/wav2lip/source/temp/pred.avi', args.pred_path)
			subprocess.call(command, shell=platform.system() == 'Windows')
