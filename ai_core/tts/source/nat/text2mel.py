...
def load_lexicon(fn):
    lines = open(fn, "r", encoding='utf-8', errors='ignore').readlines()
    lines = [l.lower().strip().split("\t") for l in lines]
    return dict(lines)


def predict_duration(tokens, vocal = "female", language = "VI"):
    def fwd_(x):
        return DurationModel(is_training=False)(x)

    forward_fn = jax.jit(hk.transform_with_state(fwd_).apply)
    with open(FLAGS.ckpt_dir / f"{language}/{vocal}/nat/duration_latest_ckpt.pickle", "rb") as f:
        dic = pickle.load(f)
    x = DurationInput(
...
def predict_mel(tokens, durations, vocal = "female", language = "VI"):
    ckpt_fn = FLAGS.ckpt_dir / f"{language}/{vocal}/nat/acoustic_latest_ckpt.pickle"
...
def text2mel(
    text: str, lexicon_fn=FLAGS.data_dir / "lexicon.txt", silence_duration: float = -1.0, vocal = "female", language = "VI"
):
    tokens = text2tokens(text, lexicon_fn)
    durations = predict_duration(tokens, vocal, language= language)
    durations = jnp.where(
    ...
    mels = predict_mel(tokens, durations, vocal, language= language)
